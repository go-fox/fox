...
func (t *Template) Execute(data interface{}) (*bytes.Buffer, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, fmt.Sprintf("template parse error:%s", t.text))
	}

	buf := new(bytes.Buffer)
	if err = tem.Execute(buf, data); err != nil {
		return nil, errors.Wrap(err, fmt.Sprintf("template execute error:%s", t.text))
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, fmt.Sprintf("go format error:%s", buf.String()))
	}
